...
$list_users = $CRUD->Written("SELECT * FROM staff
                            WHERE staff_status = 1",null,true);
foreach ($list_users as $key => $value) 
{
    ...
    $pending_d = "";
    $pending_e = "";
    $list_tasks = $CRUD->Written("SELECT *, DATEDIFF(task_duedate,'".date("Y-m-d")."') AS date_remaining FROM tasks_tasks WHERE (staff_id=$staff_id OR FIND_IN_SET('$staff_id',task_members) AND DATEDIFF(task_duedate,'".date("Y-m-d")."')=0 AND task_completed=0  AND task_status=0",null,true);
    if(!empty($list_tasks)){
