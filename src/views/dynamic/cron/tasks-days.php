...
$list_users = $CRUD->Written("SELECT * FROM staff WHERE staff_status = 1",null,true);
...
foreach ($list_users as $key => $value) 
{
    ...
    $list_tasks = $CRUD->Written("SELECT *, DATEDIFF(task_duedate,'".date("Y-m-d")."') AS date_remaining FROM tasks_tasks WHERE (staff_id=$staff_id OR FIND_IN_SET('$staff_id',task_members)) AND DATEDIFF(task_duedate,'".date("Y-m-d")."')<=1 AND task_completed=0 AND task_status=0",null,true);
    if(!empty($list_tasks)){
        foreach ($list_tasks as $key => $val) {
            $task_id = $val["task_id"];
            $arrayTask[] = $task_id; 
            $arraySubTask = [];
            $whereSubtask = " AND staff_id = ".$staff_id;
            if(  $staff_id = $val["staff_id"] ){ 
                $whereSubtask = "";
            }
            $list_subTasks = $CRUD->Written("SELECT * FROM tasks_subtasks WHERE task_id=$task_id AND (subtask_completed IS NULL OR subtask_completed=0) AND deleted = 0".$whereSubtask,null,true);
        ...
        $res = $CRUD->Written("SELECT notif_id FROM notifications WHERE task_id = '$taskA' AND staff_id = $staff_id AND DATE(notif_date_sent) >= '".date("Y-m-d")."'",null,true);
        if( empty($res) && !empty($value['staff_email']) ){
            unset($_Valores);
            $_Valores["staff_id"]         = $value["staff_id"];
